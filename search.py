...
def with_engine(on_fail_return=None):
    ...
    def decorator(func: Callable):
        ...
        def wrapper(self, vendor_or_result, *args, **kwargs):
            ...
            # --- DEBUGGING LOGS ---
            logger.info(f"[DEBUG] Decorator received vendor: {vendor} (type: {type(vendor)})")
...
class Search:
    ...
    def search(self, engine: SearchEngine, vendor: Vendor, search_term: str) -> List[SearchResult]:
        logger.info(f"Delegating search for '{search_term}' to {vendor.value} engine...")
    ...
    def get_fully_hydrated_search_result(
        ...
        logger.info(f"Delegating hydration for '{search_result.lcsc_id}' to {search_result.vendor.value} engine...")
        return engine.get_fully_hydrated_search_result(search_result)

    @with_engine(on_fail_return=None)
    def download_image_from_url(self, engine: SearchEngine, vendor: Vendor, image_url: str) -> Optional[tuple[bytes, str]]:
