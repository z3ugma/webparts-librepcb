        ...
        try:
            ...
            if module_name == "workbench":
                assert hasattr(module, "main"), (
                    f"Module {module_name} should have main() function"
                )
            elif main_class:
                # Just verify we can access the class (don't instantiate to avoid Qt setup issues)
                assert callable(main_class), (
                    f"Main class in {module_name} should be callable"
                )
            else:
                # At minimum, the module should import without error
                assert module is not None, (
                    f"Module {module_name} should import successfully"
                )
    ...
    def test_ui_xml_syntax(self, ui_file):
        ...
        try:
            ...
            assert root.get("version") == "4.0", (
                f"UI version should be 4.0 in {ui_file}"
            )

            # Should have a class definition
            classes = root.findall("class")
            assert len(classes) >= 1, (
                f"UI file {ui_file} should define at least one class"
            )

            # Should have a widget definition
            widgets = root.findall("widget")
            assert len(widgets) >= 1, (
                f"UI file {ui_file} should define at least one widget"
            )
            ...
            if ui_file_path.exists():
                try:
                    ...
                    assert widget is not None, (
                        f"QUiLoader should be able to load {ui_file}"
                    )
    ...
    def test_all_ui_files_have_python_counterparts(self):
        ...
        for ui_file in ui_files:
            ...
            assert py_file.exists(), (
                f"UI file {ui_file.name} should have corresponding Python file {py_file.name}"
            )
