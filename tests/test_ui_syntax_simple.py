...
def test_ui_modules_syntax_check():
    ...
    for module in ui_modules:
        ...
        cmd = [sys.executable, "-c", f"import {module}; print('{module} syntax OK')"]
        ...
        try:
            ...
            assert result.returncode == 0, (
                f"Import failed for {module}: {result.stderr}"
            )
            assert "syntax OK" in result.stdout, (
                f"Expected success message not found for {module}"
            )
...
def test_workbench_main_import():
    ...
    try:
        ...
        assert "Workbench syntax OK" in result.stdout, (
