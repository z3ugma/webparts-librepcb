...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QFrame, QGraphicsView, 
    QGraphicsScene, QGraphicsPixmapItem, QGraphicsTextItem, QLabel, 
...
class LibraryElementPage(QWidget):
    ...
    def _find_widgets(self):
        ...
        self.review_stack = self.ui_content.findChild(QStackedWidget, "reviewStackedWidget")
        self.part_info_widget = self.context_frame.findChild(PartInfoWidget, "part_info_widget")
        
        # Replace hero image placeholder with ZoomPanGraphicsView
        hero_view_placeholder = self.context_frame.findChild(QGraphicsView, "image_hero_view")
        ...
        self.back_to_library_button = self.context_frame.findChild(QPushButton, "back_to_library_button")
        self.button_PreviousStep = self.context_frame.findChild(QPushButton, "button_PreviousStep")
        self.button_NextStep = self.context_frame.findChild(QPushButton, "button_NextStep")
        self.page_FootprintReview: FootprintReviewPage = self.review_stack.findChild(QWidget, "page_FootprintReview")
        self.page_SymbolReview: SymbolReviewPage = self.review_stack.findChild(QWidget, "page_SymbolReview")
    ...
    def _connect_signals(self):
        ...
        self.step_labels = [self.context_frame.findChild(ClickableLabel, f"step{i+1}_Status") for i in range(4)]
    ...
    def set_component(self, component):
        ...
        if self.part_info_widget: self.part_info_widget.set_component(component)
        ...
        if hasattr(component, 'image_url'):
            ...
            hero_path = LibrePCBElement.PACKAGE.dir.parent / "webparts" / component.uuid / WebPartsFilename.HERO_IMAGE.value
            if hero_path.exists(): self._set_hero_pixmap(QPixmap(str(hero_path)))
            else: self._set_hero_text(UIText.IMAGE_NOT_AVAILABLE.value)

        footprint_pixmap = QPixmap(component.footprint.png_path) if hasattr(component, 'footprint') and component.footprint.png_path else QPixmap()
        self.page_FootprintReview.set_footprint_image(footprint_pixmap)
        symbol_pixmap = QPixmap(component.symbol.png_path) if hasattr(component, 'symbol') and component.symbol.png_path else QPixmap()
    ...
    def go_to_step(self, index):
        ...
        if self.review_stack and 0 <= index < self.review_stack.count():
            ...
            logger.info(f"Navigated to workflow step {index + 1}: '{self.step_label_text[index]}'.")
            
            # Enable/disable navigation buttons
            if self.button_PreviousStep: self.button_PreviousStep.setEnabled(index > 0)
            if self.button_NextStep: self.button_NextStep.setEnabled(index < self.review_stack.count() - 1)
            
            # Update label styles
            for i, label in enumerate(self.step_labels):
                if label:
                    is_active_step = (i == index)
                    new_text = f"<b>{self.step_label_text[i]}</b>" if is_active_step else self.step_label_text[i]
                    if label.text() != new_text:
                        label.setText(new_text)
                        logger.debug(f"Set step {i + 1} label to '{'bold' if is_active_step else 'plain'}'.")
        else:
            logger.warning(f"Could not navigate to step {index + 1}: Index out of range or review_stack not found.")

    def next_step(self):
        if self.review_stack: self.go_to_step(self.review_stack.currentIndex() + 1)
