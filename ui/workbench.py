...
class ComponentWorker(QObject):
    ...
    def hydrate_search_result(self, result: SearchResult):
        try:
            self.hydration_completed.emit(self.api_service.get_fully_hydrated_search_result(result))
        except Exception as e:
...
class WorkbenchController(QObject):
    ...
    def _find_widgets(self):
        ...
        self.page_LibraryElement: LibraryElementPage = self.window.findChild(QWidget, "page_LibraryElement")
        
        self.pages = {"library": self.page_Library, "search": self.page_Search, "library_element": self.page_LibraryElement}
    ...
    def _connect_signals(self):
        ...
        self.page_Search.add_to_library_requested.connect(self.go_to_library_element)
    ...
    def go_to_library_element(self):
        if self.current_search_result:
            ...
            self.window.statusBar().showMessage(f"Reviewing {self.current_search_result.part_name}", 2000)
        else:
            self.window.statusBar().showMessage("Please select a part to add to the library first.", 3000)
    ...
    def on_hydration_completed(self, result: SearchResult):
        ...
        self.page_Search.set_symbol_image(QPixmap(result.symbol_png_path) if result.symbol_png_path else QPixmap())
        self.page_Search.set_footprint_image(QPixmap(result.footprint_png_path) if result.footprint_png_path else QPixmap())
    ...
    def on_search_item_selected(self, result: SearchResult):
        ...
        self.page_Search.add_to_library_button.setEnabled(result is not None)
